...
void genetic :: Initialize(int argc, char ** argv){
	...
	generator = new TRandom3(0); // Random numbers
...
void genetic :: Generazione(int num){
	...
	Mutazioni();	// Applico mutazioni ai figli
...
// QuickSort: arr[] --> array, low  --> starting index, high  --> ending index
...
int genetic :: Partition(double *arr, int low, int high){
    ...
    for(int j = low; j <= high-1; j++){ 
        if(arr[j] <= pivot){	// If current element is smaller than or equal to pivot, increment index of smaller element
            i++;    			 
            Swap(arr, i, j); 
        } 
    } 
...
void genetic :: PrintConfig(string rnk){

	ofstream WriteConfig(("best_conf"+rnk+".txt").c_str());
	ofstream WriteBest(("walks"+rnk+".dat").c_str());
	ofstream WriteAvg(("averages"+rnk+".dat").c_str());
