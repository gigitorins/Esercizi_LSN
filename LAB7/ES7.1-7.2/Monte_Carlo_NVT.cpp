...
void mcNVT :: Move(void){
	...
	for(int i=0; i<npart; ++i){
		...
		xold = x[o], yold = y[o], zold = z[o];				// Old
		energy_old = Boltzmann(xold,yold,zold,o);
		xnew = Pbc( x[o] + delta*(rnd.Rannyu() - 0.5) );	// New
		ynew = Pbc( y[o] + delta*(rnd.Rannyu() - 0.5) );
		znew = Pbc( z[o] + delta*(rnd.Rannyu() - 0.5) );
		energy_new = Boltzmann(xnew,ynew,znew,o);

		p = exp(beta*(energy_old-energy_new));				// Metropolis test
		if(p >= rnd.Rannyu()){
			x[o] = xnew, y[o] = ynew, z[o] = znew;			// Update
			accepted += 1.0;
		}
		attempted += 1.0;
	}
	...
	for(int i=0; i<npart-1; ++i){
		for (int j=i+1; j<npart; ++j){
            ...
            for(int k=igofr; k<igofr+nbins; ++k){
                ...
                if(dr >= inf && dr < sup)	walker[k] += 2;		// Check if dr is in the k-th interval
            }
